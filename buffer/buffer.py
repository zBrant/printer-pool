...
def handle_clients(client_socket: socket.socket, client_address: str, document_handler: DocumentHandler, client_id: int) -> None:
    try:
        while True:
            request = client_socket.recv(1024)
            if not request: raise Exception()
            json_message = json.loads(request.decode('utf-8'))

            if json_message['message'] == 'get file':
                Printer.send_message(document_handler.get_document(), client_socket)
            else:
                document_handler.add_to_docs_queue(json_message, client_id)
...
def run_server() -> None:
    ...
    try:
        ...
        document_handler = DocumentHandler(int(m_value), threading, LogHandler())

        while True:
            client_socket, client_address = server.accept()
            client_id += 1
            LogHandler.generic_log(f'Accepted connection from {client_address[0]}:{client_address[1]}')
            threading.Thread(target=handle_clients, args=(client_socket,client_address,document_handler, client_id)).start()
    except Exception as e:
        server.close()
        LogHandler.generic_log(f'ERROR: {e}', exit_program=True)
